function montarBarra() {
  ...
  fetch('/Demeter/barra_navegacion.html')
    .then(function(res) {
      if (!res.ok) { return null; }// Si hay un error, retorna null
      return res.text();
    })
    .then(function(html) {
      if (!html) { return; }// Si no se obtuvo HTML, sale de la función
    ...
    .then(function(modulo) { // Importa el módulo de autenticación
      if (!modulo || !modulo.DemeterAuth) { return; }
      var auth = new mod.DemeterAuth();

      var btnContainer = host.querySelector('.btn-container'); //verifica que btn-container exista antes de usarlo
      if (!btnContainer) { return; }

      if (!auth.estaAutenticado()) {// Si no está autenticado, muestra los botones de login y registro 
      ...
      if (logoutBtn) {
        logoutBtn.addEventListener('click', function(e) {
          ...
          if (auth && auth.cerrarSesion) { auth.cerrarSesion(); }
        });
      }
    })
    .catch(function() {});
